    ...
    onResize() {
        ...
        if (this.ctx.canvas.width >= this.ctx.canvas.height) {
            for (let i = 0; i < this.TILE_NUMBER; i++) {
                ...
                this.points.push(
                    new Point(
                        (stop.x / this.TILE_NUMBER) * i + start.x,
                        (Math.sin(temp) * stop.y) / 5 + stop.y / 2.5
                    )
                );
            }
        } else {
            for (let i = 0; i < this.TILE_NUMBER; i++) {
                ...
                this.points.push(
                    new Point(
                        (Math.sin(temp) * stop.x) / 5 + stop.x / 2.5,
                        (stop.y / this.TILE_NUMBER) * i + start.y
                    )
                );
            }
        }
        ...
        this.turtlePositions.forEach(e => {
            ...
            if (this.ctx.canvas.width >= this.ctx.canvas.height) {
                if (e.mapPosition === 0) {
                    // pozycje startowe
                    const x =
                        this.points[e.mapPosition].x + 10 - this.TurtleSize / 2;
                    const y =
                        this.points[e.mapPosition].y +
                        (this.TurtleSize + 5) * e.colour -
                        this.TurtleSize * 2.5;
                    ...
                    // reszta pól
                    const x =
                        this.points[e.mapPosition].x + this.TurtleSize / 2;
                    const y =
                        this.points[e.mapPosition].y +
                        this.TurtleSize / 2 -
